...
def VND(sort_data, peso_tot, data, num_obj):
    ...
    S, agg_weight = greedy_mochila(sort_data, peso_tot, sort_data, False)
    j=0
    while (j <= 2):
        S_prima = busquedalocal(sort_data, peso_tot, sort_data, num_obj, False, j)
        if C(S_prima) < C(S):
            S = S_prima
            j = 0
        else:
...
def GVNS(sort_data, peso_tot, data, num_obj):
    ...
    S_segunda = VND(sort_data, peso_tot, S_prima, num_obj)
    print("S_sgunda:"+str(S_segunda))
    print("S:"+str(S))
    print(S[0])
    while (n < n_max and C(S_segunda) < C(S[0])):
        j = 1
        while (j <= 2):
            S = S_segunda
            entorno = int(random.choice([0, 1, 2]))
